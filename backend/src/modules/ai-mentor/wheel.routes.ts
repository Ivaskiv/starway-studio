...
router.post('/', authRequired, async (req, res) => {
  try {
    ...
    const result = await sql`
      ...
      VALUES (${user_id}, ${JSON.stringify(scores)}, ${total_score}, ${avgScore}, ${JSON.stringify(
        strengths,
      )}, ${JSON.stringify(gaps)})
      RETURNING *
    `.then(r => r[0]);
    ...
    res.json({
      ...
      average: parseFloat(result.average_score),
...
router.get('/cooldown', authRequired, async (req, res) => {
  try {
    const row = await sql`
      SELECT created_at FROM wheel_assessments WHERE user_id = ${
        req.user!.id
    ...
    res.json({
      canFill: daysSince >= COOLDOWN_DAYS,
      daysLeft: Math.max(0, COOLDOWN_DAYS - daysSince),
      lastFilledAt: row.created_at,
    });
  } catch {
    res.json({ canFill: true, daysLeft: 0, lastFilledAt: null });
  }
});
...
router.get('/latest/:user_id', authRequired, async (req, res) => {
  try {
    ...
    res.json({
      ...
      average_score: parseFloat(w.average_score),
