...
export async function chatMentor(user: { id: string; first_name?: string }, input: MentorRequest) {
  const user_id = user.id;

  // 1️⃣ історія діалогу
  const history = await getUserContext(user_id);

  const chat_history: Array<{ role: 'user' | 'assistant'; content: string }> = history
    .filter(h => h.text)
    .map(h => ({
  ...
  // 4️⃣ AI відповідь
  const aiText = await generateMentorChatResponse(input.prompt, chat_history, context);
