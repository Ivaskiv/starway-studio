...
const DOJIM_72H_BY_SEGMENT: Record<AbTestResultKey, FollowupCopy> = {
  ...
  action: {
    ...
    body: [
      '{firstName}.',
      '',
      'Ксенія робила багато — контент, навчання, нові проєкти. Але',
      'відчуття що ходить по колу не минало.',
      '',
      'На практиці побачила що кожна її дія починається з нуля — без',
      'розуміння куди веде.',
      '',
      'Після практики вперше зробила одну дію — але точну. Ту що',
      'насправді мала сенс.',
      '',
      'Каже: «Я не шукаю дешевших шляхів — я шукаю результат. І вкладаю',
      'не в навчання, а вкладаю в себе.»',
      '',
      'Саме такі ситуації ми розбираємо на Zoom-практиках.',
      '',
      AB_TEST_FOCUS_PRICE_SUMMARY,
...
Може ти вже писала списки бажань. Проходила курси. Питала себе
чого хочеш.

Більшість наших учасниць приходили саме з думкою: «Я вже все це
проходила.»

Можливо тому ти досі тут і читаєш це повідомлення.

Різниця одна: там ти отримувала інформацію. Тут ми працюємо з
твоєю конкретною ситуацією — і ти нарешті чуєш себе а не чужі
поради.
...
Різниця одна: подруги кажуть що обрати. Тут ми дивимось чого ти
насправді боїшся втратити — і вибір відбувається сам. Без тиску.
...
Різниця одна: знати і зробити — це різні речі. Тут ми знаходимо
що саме тебе тримає між «вирішила» і «зробила» — і перший крок
відбувається ще під час практики.
...
Може ти вже брала нові курси. Читала книги про продуктивність.
...
Різниця одна: більше дій — не вихід. Тут ми знаходимо де саме все
розсипається — і замість списку нових дій ти виходиш з одним
кроком. Але точним. Тим що нарешті веде до результату.

Тому багато учасниць після першої практики вперше відчувають що
бачать результат, а не просто зайняті.
...
// TODO(STEP B): dojim_7d_review screenshot однаковий для всіх
// сегментів, поки нема реальних Drive ID для goal/choice/decision/action
...
Через тиждень після тесту більшість людей повертаються до
звичного життя і відкладають це ще на кілька місяців.

Ти вже знаєш що тебе зупиняє. Це не так часто буває — що людина
бачить це чесно.

Нижче відгук після першої практики.

${AB_TEST_DOJIM_7D_REVIEW_QUOTE_GOAL}

Почати можна з одного місяця. Це 15 євро — менше ніж одна
консультація.
...
${AB_TEST_FOCUS_PRICE_SUMMARY}`,
    ...
    blocks: mergeTextBlocks([
      ...
      telegramBlock.quote(AB_TEST_DOJIM_7D_REVIEW_QUOTE_GOAL),
      telegramBlock.image(AB_TEST_SCREENSHOT_URLS.dojim_7d_review),
      telegramBlock.text('Почати можна з одного місяця. Це 15 євро — менше ніж одна консультація.'),
      telegramBlock.pricing('ФОКУС | Zoom-практики AB System'),
      ...AB_TEST_FOCUS_TARIFF_BLOCKS,
...
${AB_TEST_DOJIM_7D_REVIEW_QUOTE_CHOICE}
...
${AB_TEST_FOCUS_PRICE_SUMMARY}`,
    ...
    blocks: mergeTextBlocks([
      ...
      telegramBlock.quote(AB_TEST_DOJIM_7D_REVIEW_QUOTE_CHOICE),
      telegramBlock.image(AB_TEST_SCREENSHOT_URLS.dojim_7d_review),
      telegramBlock.text('Почати можна з одного місяця. Це 15 євро — менше ніж одна консультація.'),
      telegramBlock.pricing('ФОКУС | Zoom-практики AB System'),
      ...AB_TEST_FOCUS_TARIFF_BLOCKS,
...
${AB_TEST_DOJIM_7D_REVIEW_QUOTE_DECISION}
...
${AB_TEST_FOCUS_PRICE_SUMMARY}`,
    ...
    blocks: mergeTextBlocks([
      ...
      telegramBlock.quote(AB_TEST_DOJIM_7D_REVIEW_QUOTE_DECISION),
      telegramBlock.image(AB_TEST_SCREENSHOT_URLS.dojim_7d_review),
      telegramBlock.text('Почати можна з одного місяця. Це 15 євро — менше ніж одна консультація.'),
      telegramBlock.pricing('ФОКУС | Zoom-практики AB System'),
      ...AB_TEST_FOCUS_TARIFF_BLOCKS,
...
${AB_TEST_DOJIM_7D_REVIEW_QUOTE_ACTION}
...
${AB_TEST_FOCUS_PRICE_SUMMARY}`,
    ...
    blocks: mergeTextBlocks([
      ...
      telegramBlock.quote(AB_TEST_DOJIM_7D_REVIEW_QUOTE_ACTION),
      telegramBlock.image(AB_TEST_SCREENSHOT_URLS.dojim_7d_review),
      telegramBlock.text('Почати можна з одного місяця. Це 15 євро — менше ніж одна консультація.'),
      telegramBlock.pricing('ФОКУС | Zoom-практики AB System'),
      ...AB_TEST_FOCUS_TARIFF_BLOCKS,
...
function buildBranchCopy(segment: AbTestResultKey): BranchFollowupCopy {
  ...
  return {
    ...
    RESULT_FOLLOWUP_48H: buildFocusFollowup(DOJIM_48H_BY_SEGMENT[segment]),
    RESULT_FOLLOWUP_72H: buildFocusFollowup(DOJIM_72H_BY_SEGMENT[segment]),
    RESULT_DOJIM_24H: buildFocusFollowup(dojim24h),
    RESULT_DOJIM_48H: buildFocusFollowup(DOJIM_48H_BY_SEGMENT[segment]),
    RESULT_DOJIM_72H: buildFocusFollowup(DOJIM_72H_BY_SEGMENT[segment]),
    RESULT_DOJIM_5D: buildFocusFollowup(DOJIM_5D_BY_SEGMENT[segment]),
    RESULT_DOJIM_7D: buildFocusFollowup(DOJIM_7D_BY_SEGMENT[segment]),
  }
}
