...
// backend/src/products/ab-system/telegram/handler.ts
...
} from '../../../modules/telegram-mentor/services/identity/pending.js'
import {
  planAck,
...
export async function handleAbTestAnswer(
  ...
  const { sendQuestionDirect } = await import('./views.js')
...
export async function handleAbTestRestart(
  ...
  const { startAbTestFlow } = await import('./service.js')
...
export async function handleShowInside(
  ...
  const { dispatchAbTestPracticeSequence } = await import('./views.js')
