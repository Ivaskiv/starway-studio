...
router.post('/connect', authRequired, async (req, res) => {
  try {
    const user_id = req.user!.id;
    const { provider, external_id, username, accessToken, refreshToken, metadata } =
      req.body as SocialConnection;
...
// type SocialProvider = 'telegram' | 'instagram' | 'facebook' | 'whatsapp';
...
//     const user_id = req.user!.id;
//     const connections = await sql`
//       SELECT provider, external_id, username, connectedAt, metadata
//       FROM user_social_connections
//       WHERE user_id = ${user_id}
//       ORDER BY connectedAt DESC
//     `;
...
//     if (!provider || !external_id) return res.status(400).json({ error: 'invalid_data', message: "Provider та external_id обов'язкові" });

//     const existing = await sql`SELECT user_id FROM user_social_connections WHERE provider = ${provider} AND external_id = ${external_id}`;
//     if (existing.length && existing[0].user_id !== user_id) {
//       return res.status(409).json({ error: 'already_connected', message: 'Акаунт вже підключено до іншого користувача' });
//     }

//     await sql`
//       INSERT INTO user_social_connections (user_id, provider, external_id, username, accessToken, refreshToken, metadata, connectedAt)
//       VALUES (${user_id}, ${provider}, ${external_id}, ${username || null}, ${accessToken || null}, ${refreshToken || null}, ${JSON.stringify(metadata || {})}, NOW())
//       ON CONFLICT (user_id, provider)
//       DO UPDATE SET
//         external_id = ${external_id},
//         username = ${username || null},
...
//     if (provider === 'telegram') {
//       await sql`UPDATE users SET telegram_id = ${external_id}, telegram_user_name = ${username || null}, telegram_connected_at = NOW() WHERE id = ${user_id}`;
//     }
...
//     const user_id = req.user!.id;
//     const { provider } = req.body;
//     if (!provider) return res.status(400).json({ error: 'invalid_data', message: "Provider обов'язковий" });

//     await sql`DELETE FROM user_social_connections WHERE user_id = ${user_id} AND provider = ${provider}`;
//     if (provider === 'telegram') {
//       await sql`UPDATE users SET telegram_id = NULL, telegram_user_name = NULL, telegram_connected_at = NULL WHERE id = ${user_id}`;
//     }
...
//       INSERT INTO telegram_link_codes (user_id, code, expires_at)
//       VALUES (${user_id}, ${linkCode}, NOW() + INTERVAL '10 minutes')
//       ON CONFLICT (user_id) DO UPDATE SET code = ${linkCode}, expires_at = NOW() + INTERVAL '10 minutes'
//     `;

//     res.json({ success: true, link: `https://t.me/${botUsername}?start=${linkCode}`, expiresIn: 600 });
//   } catch (err: any) {
//     console.error(err);
//     res.status(500).json({ error: 'server_error', message: 'Помилка сервера' });
//   }
