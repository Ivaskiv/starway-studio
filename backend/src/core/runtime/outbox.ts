...
import { buildRuntimeTelemetry, claimRuntimeEventReplay, withRuntimeAdvisoryLock, type RuntimeIdempotencyInput } from './idempotency.js'
import { processZoomAudioOutboxItem } from './zoom/audio.js'
...
export type RuntimeOutboxProcessItem = {
  id: string
  scope: string
  type: string
  source: string
  ...
  return withRuntimeAdvisoryLock({
    ...
    for (const item of due) {
      ...
      if (item.type === 'ZOOM_AUDIO_UPLOADED') {
        const zoomResult = await processZoomAudioOutboxItem(item, payload)
        if (zoomResult === 'failed') continue
      }
