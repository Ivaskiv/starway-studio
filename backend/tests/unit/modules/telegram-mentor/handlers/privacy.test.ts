...
vi.mock('../abTest.start.ts', () => ({
  welcomeMessage: (...args: unknown[]) => mockWelcomeMessage(...args),
}))

vi.mock('../homeScreen.builder.ts', () => ({
...
vi.mock('../lidmagnet.ts', () => ({
  handleLidmagnet: vi.fn(),
}))

vi.mock('../../flows/leadMagnet.flow.ts', () => ({
...
vi.mock('../../../../core/state-machine/ctaFoundation.ts', () => ({
...
import { dispatchTelegramCallbackEvent } from '../../services/delivery/event-bus/index.ts'
import { handlePrivacy, handlePrivacyBack } from '../privacy.ts'
