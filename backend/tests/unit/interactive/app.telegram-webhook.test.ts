...
} from '../telegramWebhookSync.ts'
import { startInteractiveTelegramConsumers } from '../telegramConsumerStartup.ts'
...
describe('telegram webhook contract', () => {
  ...
  it('routes main secret to the main bot', async () => {
    const { resolveTelegramWebhookTarget } = await import('../../app.ts')
