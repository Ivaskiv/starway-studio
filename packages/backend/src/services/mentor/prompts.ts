// packages/backend/src/services/mentor/prompts.ts

import { ChatMessage, EveningResponse, MorningResponse, SessionContext } from "../../types/types";
...
export async function generateMorningSession(
  ...
  const wheel_context = context.wheel_scores
    ?.map(s => `${s.category_id}: ${s.score}/10`)
    .join(', ') || 'немає даних'
...
ТІЛЬКИ JSON.`
  ...
  try {
    ...
    return {
      ...
      completion_rate: Math.round((completed_actions.length / Math.max(totalSessions * 3, 1)) * 100),
