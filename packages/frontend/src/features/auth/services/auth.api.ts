...
export interface MeResponse {
  user: User & UserLevel & {
    ...
    // ✅ Subscription fields доступні через User
  };
}
...
export const authApi = api.injectEndpoints({
  endpoints: (builder) => ({
     ...
     */
    connectTelegram: builder.mutation<{ success: boolean }, { telegramId: string; username?: string }>({
      query: (data) => ({
        url: '/auth/telegram/connect',
        method: 'POST',
        body: data,
      }),
      invalidatesTags: ['User'],
     ...
     */
    disconnectTelegram: builder.mutation<{ success: boolean }, void>({
      query: () => ({
        url: '/auth/telegram/disconnect',
        method: 'POST',
      }),
      invalidatesTags: ['User'],
    }),
