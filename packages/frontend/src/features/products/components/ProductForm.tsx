...
import { PRODUCT_TYPE_LABELS, CURRENCY_SYMBOLS } from '@/features/products/types/product.labels'
import { productToForm, formToCreatePayload, formToUpdatePayload } from '@/features/products/types/product.mappers'
import type { ProductFormInputs, ProductType, Currency, ProductFormat, ProductIntegration } from '@/features/products/types/product.types'
...
export default function ProductForm() {
  ...
  const { data: product, isLoading: isLoadingProduct } = useGetProductByIdQuery(id!, { skip: !isEdit })
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormFieldController name="type" label="Тип продукту" type="select" options={productTypeOptions} />
        <FormFieldController name="price" label="Ціна" type="number" />
        <FormFieldController name="currency" label="Валюта" type="select" options={currencyOptions} />
        <FormFieldController name="format" label="Формат доставки" type="select" options={formatOptions} />
        <FormFieldController name="integration" label="Основна інтеграція" type="select" options={integrationOptions} />
        ...
        {/* Чекбокси */}
        <FormFieldController name="includesTrial" label="Включити пробний період (7 днів)" type="checkbox" />
        <FormFieldController name="includesMentorship" label="Включити індивідуальне наставництво" type="checkbox" />
