// packages/frontend/src/features/ai-mentor/components/DailySession.tsx
...
const ListQuestion = ({ question, value, onChange }: QuestionProps) => {
  const items = (value as string[]) || Array(question.count || 3).fill('');
    ...
    if (isLastStep) {
      try {
        if (type === 'morning') {
          await submitMorning({ 
            user_id, 
            answers: answers as MorningSessionAnswers 
          }).unwrap();
          ...
          await submitEvening({ 
            user_id, 
            answers: answers as EveningSessionAnswers 
          }).unwrap();
        }
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, x: -20 }}
          transition={{ duration: 0.3 }}
          className="rounded-3xl p-6 mb-6 bg-gradient-to-br from-gray-900/80 to-gray-800/80 backdrop-blur-xl border border-white/10 shadow-2xl"
        >
          {currentQuestion && (
            <>
