...
export default function ProductBuilder() {
  ...
  const addBlock = (type: BlockType) => {
    ...
    const newBlock: Block = {
      ...
      settings: type === 'task' ? { task_type: 'text' } : {}
    };
  ...
  const generateWithAI = async () => {
    ...
    try {
      const typeLabels: Record<ProductForm['type'], string> = {
        course: 'курсу',
        practicum: 'практикуму',
        ai_mentor: 'AI-ментора'
      };
      ...
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `Створи структуру ${typeLabels[productForm.type]} "${productForm.title}" на ${productForm.duration_days} днів.
            ...
            [{"type":"video","title":"","content":"","duration":15,"points":20}]`
          }]
        })
      });
  ...
  const saveProduct = async () => {
    ...
    try {
      const payload: SaveProductRequest = { ...productForm, blocks };
  ...
  const getBlockColor = (type: string) => 
    BLOCK_COLORS[type] || 'border-gray-700 bg-gray-900';

  const totalPoints = blocks.reduce((sum, b) => sum + (b.points || 0), 0);
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-green-500">{totalPoints}</span>
                </div>
              </div>
            </div>
          </div>
                ...
                <>
                  <div className="space-y-4 mb-6">
                    {blocks.map((block, index) => (
                      <div key={block.id} className={`border-2 rounded-xl p-4 ${getBlockColor(block.type)}`}>
                  ...
                  <Button variant="success" onClick={saveProduct} className="w-full py-4">
