import type { GeminiRetryPolicyConfig, IGeminiRateLimitHandler, IGeminiRetryPolicy } from './types.js'
...
export class RetryPolicy implements IGeminiRetryPolicy {
  ...
  constructor(
    private readonly rateLimitHandler: IGeminiRateLimitHandler,
    private readonly sleep: (ms: number) => Promise<void> = (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
    config: GeminiRetryPolicyConfig = {},
  ) {
...
function isRetryable(error: unknown, rateLimitHandler: IGeminiRateLimitHandler): boolean {
