...
export class OpenAIRequestMapper implements IOpenAIRequestMapper {
  constructor(
  ...
  map(input: OpenAIExecuteInput): OpenAIProviderRequest {
    const promptOptions = getOpenAIOptions(input.prompt.metadata)
    const contextOptions = getOpenAIOptions(input.context.metadata)
    ...
    const executionOptions: OpenAIExecutionOptions = {
      model: getStringOption(promptOptions.model) ?? getStringOption(contextOptions.model) ?? this.defaults.defaultModel ?? DEFAULT_MODEL,
...
function buildUserMessage(input: OpenAIExecuteInput): string {
...
function buildResponseFormat(structuredOutput: OpenAIStructuredOutputConfig) {
...
function resolveStructuredOutput(
  ...
  defaultStructuredOutput: OpenAIStructuredOutputConfig | undefined,
): OpenAIStructuredOutputConfig {
...
function normalizeStructuredOutput(value: unknown): OpenAIStructuredOutputConfig | undefined {
...
function getOpenAIOptions(metadata: Record<string, unknown> | undefined): Record<string, unknown> {
  if (!metadata || typeof metadata !== 'object') {
    return {}
  }

  const direct = metadata.openai
