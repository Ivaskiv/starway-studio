...
import { getSessionCountdownLabel, getSessionDateLabel, getSessionMeta, isBattleReviewSession } from '@/features/zoom/zoom.utils'

type HomeTabProps = {
  hasTelegramInitData: boolean
  onNavigateCalendar?: () => void
  setActiveTab?: (tab: 'calendar') => void
}
...
export default function HomeTab({
  ...
  const battleCount = useMemo(
    () => sessions.filter((session) => isBattleReviewSession(session) && session.status !== 'COMPLETED').length,
    [sessions],
  )

  const bookedSessionsCount = useMemo(
    () => sessions.filter((session) => session.isMyBooking).length,
    [sessions],
  )
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-4 pt-6 pb-24">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-white/60">
              {nextSession ? getSessionCountdownLabel(new Date(nextSession.scheduledAt)) : 'Найближча Zoom-сесія ще не запланована'}
            </p>
          </div>
      ...
      <section className="rounded-3xl border border-white/10 bg-[rgba(74,144,255,0.10)] p-5">
        ...
        {nextSession ? (
          <>
            <h2 className="mt-2 text-lg font-semibold text-white">{nextSession.topic || 'ФОКУС · Zoom-практика'}</h2>
            <p className="mt-2 text-sm text-white/65">{getSessionDateLabel(nextSession.scheduledAt)}</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                ...
                className="cursor-pointer rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-semibold text-white opacity-100 transition hover:bg-white/10 active:scale-[0.98] active:bg-white/15"
              >
                Увесь календар
              </button>
            </div>
      ...
      <section className="grid grid-cols-2 gap-3">
        <ActionCard label="Календар" onClick={handleOpenCalendar} />
...
function ActionCard({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="cursor-pointer rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-4 text-left text-sm font-semibold text-white opacity-100 transition hover:bg-white/[0.08] active:scale-[0.98] active:bg-white/[0.12]"
    >
      {label}
    </button>
  )
