...
export const WheelForm = ({ userId, onComplete, onCancel, nextWheelAvailable }: WheelFormProps) => {
  ...
  const handleNext = useCallback(() => {
    ...
    void trackFrontendEvent({
      ...
      type: isIntroStep ? 'web_wheel_started' : 'web_wheel_step_advanced',
      source: 'web',
      state: 'wheel',
      payload: {
        step,
  ...
  }, [canProceed, isIntroStep, resolvedUserId, setStep, step, trackFrontendEvent])
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      void trackFrontendEvent({
        userId: resolvedUserId,
        type: 'web_wheel_submit_clicked',
        source: 'web',
        state: 'wheel',
        payload: {
          assessmentId: result.id,
          scoreCount: spheres.length,
        },
      })
