...
export const ReviewStep: React.FC<ReviewStepProps> = ({ data, template }) => {
  ...
  useEffect(() => {
    if (primaryRef.current && data?.branding?.colors?.primary) {
      primaryRef.current.style.backgroundColor = data.branding.colors.primary;
    }
    if (secondaryRef.current && data?.branding?.colors?.secondary) {
      secondaryRef.current.style.backgroundColor = data.branding.colors.secondary;
    }
  }, [data.branding.colors.primary, data.branding.colors.secondary]);
