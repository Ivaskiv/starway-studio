...
export function WheelMonthlyBoard({ onOpenInline, onOpenReports }: { onOpenInline: () => void; onOpenReports: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {isHistoryFetching ? (
        <GlassCard className="p-5 text-sm text-[var(--text-secondary)]">Оновлюємо місячну історію колеса…
      ...
      {/* {monthlyCards.map(card => (
        <GlassCard key={card.slotKey} className="p-5">
          <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
            ...
            <div className="space-y-4">
              <div className="rounded-[24px] border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[rgb(var(--accent-soft-rgb))]">Analysis</p>
                <p className="mt-2 text-sm leading-7 text-[var(--text-primary)]">{card.analysisText}</p>
              </div>

              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-[24px] border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[rgb(var(--accent-soft-rgb))]">Report</p>
                  <p className="mt-2 text-base font-semibold text-[var(--text-primary)]">
                    {card.report?.nextWeekFocus ?? 'Окремий weekly report для цього періоду ще не згенеровано'}
                  </p>
                  <p className="mt-1 text-sm leading-6 text-[var(--text-secondary)]">
                    {card.report
                      ? `${card.report.weekStart.slice(0, 10)} — ${card.report.weekEnd.slice(0, 10)} · ${card.report.completionRate ?? 0}% completion`
                      : 'Аналіз колеса вже є, але weekly report за цей місяць ще не підтягнувся.'}
                  </p>
                </div>

                <div className="rounded-[24px] border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] p-4">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[rgb(var(--accent-soft-rgb))]">Product progress</p>
