...
export interface PointsRule { id: string; action: string; points: number; description: string }
export interface Level { id: string; level: number; name: string; pointsRequired: number; rewards: string[]; badge?: string }
export interface Achievement { id: string; name: string; description: string; icon: string; condition: string; reward?: number; rarity: 'common' | 'rare' | 'epic' | 'legendary' }
export interface LeaderboardConfig { type: 'global' | 'group' | 'friends'; updateFrequency: 'realtime' | 'hourly' | 'daily'; showTop: number; rewardsEnabled: boolean; rewards?: LeaderboardReward[] }
export interface LeaderboardReward { position: number; reward: string; points?: number }
export interface StreakReward { days: number; reward: string; points?: number }
...
export interface LessonStats { lessonId: string; lessonTitle: string; views: number; completions: number; averageTime: number; rating?: number }
export interface ModuleStats { moduleId: string; moduleTitle: string; startedUsers: number; completedUsers: number; completionRate: number }
export interface PlayerStats { userId: string; userName: string; points: number; level: number; achievements: number; streak: number }
export interface AchievementStats { achievementId: string; achievementName: string; unlockedBy: number; unlockRate: number }
...
): Omit<Product, 'id' | 'createdAt' | 'updatedAt' | 'analytics' | 'creator'> & { creatorId: string } => ({
