...
export default function MiniAppPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] pb-20">
      ...
      {/* ── Pages ── */}
      {page === 'home' && <HomePage ctx={ctx} setPage={setPage} />}
      {page === 'mentor' && <MentorPage setPage={setPage} />}
...
// ── Home Page ─────────────────────────────────────────────────
function HomePage({ ctx, setPage }: { ctx: any; setPage:(p:Page)=>void }) {
  return (
...
function MentorPage({ setPage }: { setPage:(p:Page)=>void }) {
  return (
